...
\fontsize{11pt}{11pt} \selectfont {
  ...
  Portal” operated by Korea Meteorological Administration 
  (KMA)~\citep{MDOP}. To get the output data that will be paired with the weather maps, we will go 
  through the traditional procedure with either FALL3D, or Puff-UAF model.
  ...
  datasets so that it can translate the weather map image to simulation result flawlessly. In order 
  to do this tasks, we need to train model with different amount of inputs. As Korean Peninsula 
  experiencing four distinctive seasons, while we picking up the training set randomly, we would 
  better ensure that we picks a combination of data which represent the dynamic climate of Korea. 
