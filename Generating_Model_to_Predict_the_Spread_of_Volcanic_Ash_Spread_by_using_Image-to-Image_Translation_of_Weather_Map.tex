...
\title{Generating Models to Predict the Spread of Volcanic Fallout Ash%
  by using conditional Generative Adversarial Network%
  }
...
\fontsize{11pt}{11pt} \selectfont {
  ...
  economic loss, according to~\citet[p.274]{Yun_2013}, the fallout ash, the focus of this 
  ...
  causes various impairments in a wide range of areas~\citep[p.274-275]{Yun_2013}.
  ...
  dispersion. Historically, by using the Eulerian or the Lagrangian approach, according to
  ~\citet[p.3-4]{Bonadonna2012}, scientists are tried to make Volcanic Ash and Dispersion/Tracking 
  Model (VATDM). These models usually focus on predicting the pathway of fallout ash dispersion and 
  the amount of deposited ash while accepting limited variables~\citep[p. 276]{Yun_2013}.
...
\fontsize{11pt}{11pt} \selectfont {
  ...
  operated by the Korea Meteorological Administration (KMA)~\citep[from][]{MDOP}. To get the output 
  data that will be paired with the weather maps, we will go through the traditional procedure with 
...
\fontsize{11pt}{11pt} \selectfont {
  ...
  reproduce as the same result as possible compares to the original training result.

  Because of the weather maps’ style is different with the agencies that produce the map (National 
  Oceanic and Atmospheric Administration (NOAA)’s way of making the map and KMA’s way of making the 
  map is different), our model was not focused to produce an output from the maps that have been 
  publicized by different agencies. Moreover, because of the limitation of the dataset, we are only 
  able to train and test on the area near the Korean Peninsula; the models could not be verified on 
